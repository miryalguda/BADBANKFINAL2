...
const Withdraw = () => {
  ...
  const handleWithdraw = (event) => {
    ...
    if (Number(event.target.value) <= 0) {
      window.alert("Amount should be $1 to $300!");
    ...
    if (Number(event.target.value) > totalState) {
      window.alert("Withdrawal amount should not be more  than your balance!");
      return setValidTransaction(false);
    } else {
      setValidTransaction(true);
    }
    ...
    if (Number(event.target.value) >= 300) {
      window.alert("Maximum only $300 allowed  to withdraw!");
      return setValidTransaction(false);
    } else {
      setValidTransaction(true);
    }
              ...
              <div className="box1 mx-auto ">
                ...
                <div className="box">
                  <h2 style={{ color: "cornsilk" }}>
                    Your Total Balance ${totalState}
                  </h2>
                  <h2 id="total" className="card p2 m2">
                    {" "}
